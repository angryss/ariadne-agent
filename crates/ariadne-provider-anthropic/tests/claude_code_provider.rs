...
mod unix {
    use ariadne_core::{
        CompletionDelta, CompletionRequest, Message, ModelProvider, ToolDefinition,
    };
    use ariadne_provider_anthropic::ClaudeCodeProvider;
    use serde_json::json;
    use std::fs;
    ...
    async fn subscription_mode_uses_headless_cli_with_all_tools_disabled() {
        ...
        let provider = ClaudeCodeProvider::new(&program, "sonnet")
            .with_test_environment("ARIADNE_TEST_ARGS", &args)
            .with_test_environment("ARIADNE_TEST_STDIN", &stdin)
            .with_test_environment("ARIADNE_TEST_SCENARIO", "headless");
        let mut deltas = vec![];
        let result = provider
            .complete_stream(
                CompletionRequest {
    ...
    async fn subscription_mode_resolves_a_relative_executable_before_isolating_cwd() {
        ...
        let provider = ClaudeCodeProvider::new(relative_program, "sonnet")
            .with_test_environment("ARIADNE_TEST_ARGS", dir.path().join("args"))
            .with_test_environment("ARIADNE_TEST_STDIN", dir.path().join("stdin"))
            .with_test_environment("ARIADNE_TEST_SCENARIO", "headless");
    ...
    #[tokio::test]
    async fn subscription_mode_rejects_ariadne_tools_before_starting_cli() {
        let program = fixture("fake_claude.sh");
        let provider = ClaudeCodeProvider::new(program, "sonnet");
        let error = provider
            .complete(CompletionRequest {
        ...
        assert!(
            error
                .to_string()
                .contains("do not accept Ariadne tool calls")
        );
    }
    ...
    async fn subscription_mode_isolates_oauth_execution_and_streams_partial_events_once() {
        ...
        let provider = ClaudeCodeProvider::new(&program, "sonnet")
            .with_test_environment("ARIADNE_TEST_ENV", &env_file)
            .with_test_environment("ARIADNE_TEST_CWD", &cwd_file)
            ...
            .with_test_environment("ARIADNE_TEST_SCENARIO", "isolated");
        let mut deltas = vec![];
        let completion = provider
            .complete_stream(
                CompletionRequest {
    ...
    async fn subscription_mode_requires_a_successful_result_event() {
        ...
        let error = ClaudeCodeProvider::new(program, "sonnet")
            .with_test_environment("ARIADNE_TEST_SCENARIO", "no-result")
            .complete(CompletionRequest {
                messages: vec![Message::user("Hi")],
                tools: vec![],
            })
    ...
    async fn subscription_mode_rejects_an_oversized_unterminated_message() {
        let program = fixture("fake_claude.sh");
        let error = ClaudeCodeProvider::new(program, "sonnet")
            .with_test_environment("ARIADNE_TEST_SCENARIO", "oversized")
            .complete(CompletionRequest {
                messages: vec![Message::user("Hi")],
                tools: vec![],
            })
    ...
    async fn subscription_mode_applies_the_operation_deadline_to_stdin() {
        ...
        let result = tokio::time::timeout(
            ...
            ClaudeCodeProvider::new(program, "sonnet")
                ...
                .with_test_environment("ARIADNE_TEST_SCENARIO", "blocked-stdin")
                .complete(CompletionRequest {
    ...
    async fn subscription_mode_kills_a_timed_out_version_probe() {
        ...
        let result = tokio::time::timeout(
            ...
            ClaudeCodeProvider::new(program, "sonnet")
                .with_test_timeout(std::time::Duration::from_millis(100))
                .with_test_environment("ARIADNE_TEST_PID", &pid_file)
                .complete(CompletionRequest {
                    messages: vec![Message::user("Hi")],
                    tools: vec![],
                }),
    ...
    async fn subscription_mode_kills_and_reaps_after_invalid_stream_json() {
        ...
        let error = ClaudeCodeProvider::new(fixture("fake_claude.sh"), "sonnet")
            .with_test_environment("ARIADNE_TEST_SCENARIO", "invalid-json-hang")
            .with_test_environment("ARIADNE_TEST_PID", &pid_file)
            .complete(CompletionRequest {
                messages: vec![Message::user("Hi")],
                tools: vec![],
    ...
    async fn subscription_mode_kills_and_reaps_after_final_wait_timeout() {
        ...
        let error = ClaudeCodeProvider::new(fixture("fake_claude.sh"), "sonnet")
            .with_test_timeout(std::time::Duration::from_millis(100))
            .with_test_environment("ARIADNE_TEST_SCENARIO", "success-hang")
            .with_test_environment("ARIADNE_TEST_PID", &pid_file)
            .complete(CompletionRequest {
                messages: vec![Message::user("Hi")],
                tools: vec![],
