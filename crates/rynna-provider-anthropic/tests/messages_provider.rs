...
use rynna_provider_anthropic::AnthropicMessagesProvider;
use serde_json::json;
...
#[tokio::test]
async fn messages_api_round_trips_rynna_tools() {
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/v1/messages"))
        .and(body_json(json!({
            ...
            "messages":[
                ...
                {"role":"user","content":[{"type":"tool_result","tool_use_id":"call-1","content":"{\"content\":\"# Rynna\"}"}]}
            ],
