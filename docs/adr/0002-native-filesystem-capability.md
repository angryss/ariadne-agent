...
Rynna needs local filesystem access in CLI, HTTP, web, and desktop modes. Skills describe workflows but do not grant authority. MCP can supply tools, but making an external filesystem server the only implementation would add process and transport dependencies to a foundational local capability. Filesystem access also needs one consistently enforced boundary for traversal, symlinks, secrets, writes, limits, and unattended operation.

## Decision

Rynna exposes provider-neutral tool definitions and tool calls from `rynna-core`. The agent runs a bounded loop of at most eight model turns and 64 total tool calls, executes tools through a `Tool` port only when a follow-up model turn remains, and returns structured tool results to the provider. Caller-owned history remains restricted to plain user and assistant messages without internal tool metadata. Streaming buffers each turn until it is known to be final and suppresses intermediate tool-turn content.

The first implementation is an in-process `rynna-tools-filesystem` adapter activated by profile-scoped native capability names. One filesystem capability contributes read, write, exact edit, list, find, search, create-directory, and file-info tools under a canonical root.
