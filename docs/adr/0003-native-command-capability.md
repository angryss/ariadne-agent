...
- Snapshotting executable bytes removes configured-path replacement from the execution boundary, but means operators must restart Ariadne after updating a mapped executable.
- Root and every process sharing Ariadne's UID are trusted; deployments should dedicate the UID to Ariadne rather than sharing it with potentially hostile processes.
...
- Time and output limits are resource controls, not an authorization sandbox. Mapped programs can still exercise all permissions of Ariadne's OS account.
