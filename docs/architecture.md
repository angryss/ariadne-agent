...
Model providers and native tools enter through explicit core ports. The bounded agent loop sends provider-neutral tool definitions, executes at most 64 requested tools across at most seven tool-producing turns, returns structured results to the model, and reserves the eighth turn for a final answer. Streaming forwards thinking deltas immediately, while buffering user-facing content until the turn is known to be final so intermediate tool-turn content is not exposed as answer text. Skills, MCP tools, session persistence, and approval policies should follow the same boundary discipline. Durable memory now follows it through the `MemoryProvider` port. Profile-scoped skill and MCP activation is represented by safe names, but loading and execution remain future capabilities. New surfaces should compose those ports rather than duplicate orchestration.

## Durable memory

`MemoryProvider` is an optional core port for recall and retention. `Agent` validates caller history, recalls context before model execution, and retains the current prompt and successful final answer once after the tool loop. Both response modes use this path. The default `None` performs no memory I/O. Provider failures and ten-second deadlines are logged without failing chat. Recalled facts are bounded to 12,000 characters, labeled as untrusted reference data in the system message, and excluded from caller-owned history.

`rynna-config::memory` owns validated settings, a redacted response type, and atomic persistence in `memory.toml` beside provider settings. Keys omitted during updates are preserved only for the same destination; disabling memory removes the local key. The `rynna-memory-hindsight` adapter implements the Cloud/self-hosted HTTP protocol, optional bearer authentication, encoded bank path segments, bounded recall responses, async retain submission, timeouts, and sanitized errors. Redirects are disabled to avoid forwarding credentials or conversation text to another endpoint.

CLI, server, and desktop compose this adapter into all profiles. Settings are application-wide and share one explicit bank across profiles. Web/desktop save handlers replace the runtime provider after successful persistence. Copy-on-write profile registries keep in-flight requests on their original provider. Settings administration retains the HTTP server's loopback restriction. The shared React settings panel uses typed HTTP/Tauri client methods and never receives the saved key.
